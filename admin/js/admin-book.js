...
// show data
function showBookData(id) {
  let data = { data_id: id };
  $.ajax({
    type: "POST",
    url: "http://localhost/coolmaths/books/retrieve.php",
    ...
    success: function (r) {
      ...
      for (i = 0; i < r.length; ++i) {
        ...
        output +=
          ...
          "><td>" +
          (i + 1) +
          "</td><td><img class='image-preview' src='../books/" +
          r[i].img +
...
$(document).ready(function () {
  ...
  // form 0
  $("#add-books-button").on("click", ".add", function () {
    uploadCounter = true;
    const classId = $(".active-element").attr("data-cid");
    let name = "book";
    let formName = document.getElementById("book-upload-form");
    var input = formName.querySelectorAll("input");
    input[0].value = classId;
    let btn = document.getElementById("upload-book");
...
$(document).ready(function () {
  ...
  // call to function
  $("#books-tbody").on("click", ".edit", function () {
    uploadCounter = false;
    let id = $(this).attr("data-eid");
    let data = { data_id: id };
    changeStatusOfTableItems("book", data);
  });
});
