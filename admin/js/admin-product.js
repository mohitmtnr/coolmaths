...
$(document).ready(function () {
  ...
  showproductData(defaultClassId);
...
function showproductData(id) {
  ...
  $.ajax({
    type: "POST",
    url: "http://localhost/coolmaths/products/retrieve.php",
    ...
    success: function (r) {
      ...
      for (i = 0; i < r.length; ++i) {
        ...
        output +=
          ...
          "><td>" +
          (i + 1) +
          "</td><td><img class='image-preview' src='../products/" +
          r[i].img +
...
$(document).ready(function () {
  ...
  // form 0
  $("#add-products-button").on("click", ".add", function () {
    uploadCounter = true;
    const classId = $(".active-element").attr("data-cid");
    let name = "product";
    let formName = document.getElementById("product-upload-form");
    var input = formName.querySelectorAll("input");
    input[0].value = classId;
    let btn = document.getElementById("upload-product");
...
$(document).ready(function () {
  ...
  $("#products-tbody").on("click", ".edit", function () {
    ...
    let data = { data_id: id };
    changeStatusOfTableItems("product", data);
  });
});
